...
void main() {
    ...
    vec2 scaleVec = vec2(1.0);
    if (imageAspect > containerAspect) {
        scaleVec.x = containerAspect / imageAspect;
    } else {
        scaleVec.y = imageAspect / containerAspect;
    }
    
    vec2 st = vec2(vUvs.x, 1.0 - vUvs.y);
    st = (st - 0.5) * scaleVec + 0.5;
    vec2 clampedSt = clamp(st, 0.0, 1.0);
    
    vec2 atlasSt = clampedSt * cellSize + cellOffset;
    
    outColor = texture(uTex, atlasSt);
    outColor.a *= vAlpha;
}
`;
...
class PlaneGeometry extends Geometry {
  constructor(width = 1.6, height = 1) {
    super();
...
class InfiniteGridMenu {
  ...
  private mediaElements: (HTMLImageElement | HTMLVideoElement)[] = [];
  ...
  private initTexture(): void {
    ...
    Promise.all(
      this.items.map(
        item =>
          new Promise<HTMLImageElement | HTMLVideoElement>(resolve => {
            if (item.video) {
              ...
              const checkReady = () => {
                if (vid.readyState >= 3) {
                  ...
                  vid.oncanplay = null; // cleanup
                  resolve(vid);
                }
              };
              vid.oncanplay = checkReady;
              vid.load();
              checkReady();
            } else {
              const img = new Image();
      ...
      this.mediaElements = media;
      media.forEach((m, i) => {
        const x = (i % this.atlasSize) * cellSize;
        const y = Math.floor(i / this.atlasSize) * cellSize;
        ctx.drawImage(m, x, y, cellSize, cellSize);
      });
  ...
  private render(): void {
    ...
    // Update video frames in the atlas if we have videos
    if (this.mediaElements.length > 0 && this.atlasCtx && this.atlasCanvas) {
      let needsUpdate = false;
      const cellSize = 512;
      this.mediaElements.forEach((m, i) => {
        if (m instanceof HTMLVideoElement) {
          needsUpdate = true;
          const x = (i % this.atlasSize) * cellSize;
          const y = Math.floor(i / this.atlasSize) * cellSize;
          this.atlasCtx!.drawImage(m, x, y, cellSize, cellSize);
        }
      });
      if (needsUpdate) {
        gl.bindTexture(gl.TEXTURE_2D, this.tex);
    ...
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        <>
          ...
          ${
            isMoving
              ? 'bottom-[-80px] opacity-0 pointer-events-none duration-[100ms] scale-75 -translate-x-1/2'
              : 'bottom-[3.8em] opacity-100 pointer-events-auto duration-[500ms] scale-100 -translate-x-1/2'
          }
        `}
          >
            <h2 className="text-3xl md:text-5xl font-bold text-white text-center whitespace-nowrap drop-shadow-lg tracking-tight">
              {activeItem.title}
            </h2>
          </div>
        </>
